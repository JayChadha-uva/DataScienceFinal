...
After selecting various variables to display over time, it does not seem that any clear trends appear in the Beatles’ discography over time. However, there are some interesting outliers over time. For one, the album Yellow Submarine, which was released in January 1969, appears to contain some of the Beatles’ least popular songs. This makes sense because this album contains the soundtrack to its namesake film, so the music is categorically different from the Beatles’
